...
import { crearUsuarioApp, editarUsuarioApp, eliminarUsuarioApp, getUsuariosAplicacion } from '../../services/appUsersService'
import Button from '../../components/ui/Button'
import Table from '../../components/ui/Table'
import Pagination from '../../components/ui/Pagination'
import Spinner from '../../components/ui/Spinner'
import { useDiccionarioDatos } from '../../hooks/useDiccionarioDatos'
import { FALLBACKS, getAppUserAuthority, getUserStatus, labelFromKey } from '../../services/diccionarioDatos'
import ModalFormCrearEditarUsuariosAPP from '../../components/usuariosAPP/ModalFormCrearEditarUsuariosAPP'
import ModalConfirmacion from '../../components/ui/ModalConfirmacion'
import { limpiarForm } from '../../utils/formUtils'


const ListadoUsuariosAplicacion = () => {
    ...
    const [usuarioAppSeleccionado, setUsuarioAppSeleccionado] = useState(null)
    ...
    const authoritiesLabel = useDiccionarioDatos(getAppUserAuthority,FALLBACKS.getAppUserAuthority)
    
    
    const cargar = async (pagina = 0, size) => {
        setCargando(true)
        try {
            const { data } = await getUsuariosAplicacion(pagina, size)
    ...
    const handleNuevo = () => {
        setUsuarioAppSeleccionado(null)
        setModalAbiert(true)
    }

    const handleEditar = (usuario) => {
        setUsuarioAppSeleccionado(usuario)
        setModalAbiert(true)
    }
    ...
    const handleSubmit = async (form) => {
        ...
        try {
            if (usuarioAppSeleccionado) {
                await editarUsuarioApp(usuarioAppSeleccionado.id, formLimpio)
            } else {
                await crearUsuarioApp(formLimpio)
            }
            cerrarModal()
    ...
    const handleConfirmarEliminar = async () => {
        setGuardandoEntidad(true)
        try {
            await eliminarUsuarioApp(usuarioAppSeleccionado.id);
            cerrarModal()
            cargar(paginaActual, porPagina)
        } catch (error) {
    ...
    return (
        <div className="flex flex-col gap-4 ">
            ...
            {modalAbierto && (
                <ModalFormCrearEditarUsuariosAPP
                    usuarioApp={usuarioAppSeleccionado}
                    onClose={cerrarModal}
            ...
            {modalEliminarAbierto && (
                <ModalConfirmacion
                    ...
                    cargando={guardandoEntidad}>
                    <span><strong>Nombre:</strong> {usuarioAppSeleccionado?.name}</span>
                    <span><strong>Correo electrónico:</strong> {usuarioAppSeleccionado?.email}</span>
                </ModalConfirmacion>
            )
...
export default ListadoUsuariosAplicacion